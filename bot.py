...
# Use imageio-ffmpeg to provide an ffmpeg binary on Render (no apt-get needed)
try:
...
# --- Config ---
TOKEN = os.getenv("TOKEN")  # put your bot token in Render Environment Variables
...
async def start_download(u: Union[Update, CallbackQuery], ctx: ContextTypes.DEFAULT_TYPE, sendto: str):
    ...
    async def task():
        ...
        try:
            ...
            # Point yt_dlp to our bundled ffmpeg if available
            if _ffmpeg_dir:
...
async def sethelp(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # ป้องกันเฉพาะแอดมินกลุ่ม (ถ้าใช้งานในกรุ๊ป)
    try:
        chat_member = await update.effective_chat.get_member(update.effective_user.id)
        if not (chat_member.can_manage_chat or chat_member.status in ("administrator", "creator")):
            await update.message.reply_text("⛔ เฉพาะแอดมินที่ตั้งค่าได้")
            return
    except Exception:
        # ถ้าเป็นแชทส่วนตัว อนุญาตให้ตั้งได้
        pass
